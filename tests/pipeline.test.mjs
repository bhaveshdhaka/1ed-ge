...
const run = (cmd, args = [], env = {}, timeout = 60_000) => {
...
test('pre-push hook refuses push to main', () => {
  ...
  const r = run('bash', ['-c', 'echo "refs/heads/main abc123 refs/heads/main 000000" | bash .githooks/pre-push'], { HOOK_TEST_MODE: '1' })
  ok(r.code === 1, 'exit 1 on push to main')
  ok(/refusing: direct push to 'main' is blocked/.test(r.stderr), 'stderr names the refusal')
  ok(/the path is a PR to 'main'/.test(r.stderr), 'stderr points to a PR')
})

test('pre-push hook refuses push to preprod', () => {
  const r = run('bash', ['-c', 'echo "refs/heads/preprod abc123 refs/heads/preprod 000000" | bash .githooks/pre-push'], { HOOK_TEST_MODE: '1' })
  ...
  ok(/the path is a PR to 'preprod'/.test(r.stderr), 'stderr points to a PR')
})

test('pre-push hook allows push to feature branch', () => {
  const r = run('bash', ['-c', 'echo "refs/heads/feat/foo abc123 refs/heads/feat/foo 000000" | bash .githooks/pre-push'], { HOOK_TEST_MODE: '1' })
...
test('pre-push hook allows push to tag', () => {
  ...
  const r = run('bash', ['-c', 'echo "refs/tags/v0.5.0 abc123 refs/tags/v0.5.0 000000" | bash .githooks/pre-push'], { HOOK_TEST_MODE: '1' })
  ok(r.code === 0, 'exit 0 on tag push')
})

test('removed ship/sync/seed-prod scripts are gone (A4)', () => {
  for (const s of ['ship.sh', 'sync-to-prod.sh', 'sync-from-prod.sh', 'sync-branches.sh', 'deploy-prod.sh', 'deploy-test.sh', 'install-hooks.sh', 'seed-prod.sh']) {
    const r = run('bash', ['-c', `test -e scripts/${s}`])
    ok(r.code === 1, `${s} removed`)
  }
})

test('core.hooksPath points at .githooks', () => {
  const r = run('bash', ['-c', 'git config core.hooksPath'])
  ok(r.stdout.trim() === '.githooks', 'core.hooksPath = .githooks')
})

test('opencode.json denies git push main/preprod', () => {
  const r = run('bash', ['-c', 'grep -q "git push main" .opencode/opencode.json && grep -q "git push preprod" .opencode/opencode.json && echo yes'])
  ok(r.code === 0 && /yes/.test(r.stdout), 'permission map blocks main/preprod')
})
