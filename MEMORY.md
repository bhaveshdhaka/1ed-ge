...
- 2026-08-08 — **Original QA bugs: fixed, synced to PROD, deployed, verified live. Debt ledger: items 1+3 done, item 4 slice 1 done.** The owner's three complaints (day-727 counter, dot+time in News card title, `~23h` vs `halt 05:00–06:00 hkt` inconsistency) were real bugs the af1eff4 batch missed or misread. Fixed + verified on test.1ed.ge, then synced to main as a single atomic code-only commit `8dee5db` (43 files: src/ + tests/, deliberately EXCLUDING preprod filler content + MEMORY + nginx — prod stays clean-slate; per-commit history lives on the `preprod` branch). Deployed via `bash scripts/deploy.sh`, polled to 200, curl-verified live on 1ed.ge.
  ...
  - **Independent ora-2 audit verdict:** 25/27 af1eff4 claims PASS, 1 FAIL (/models h1), 1 PARTIAL (AI `ny` enum) — both since fixed. Architecture salvageable (single central functions, one logic bug, NOT per-page hardcoding).
